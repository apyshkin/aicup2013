...
public abstract class SimulatedAction {
  ...
  protected SimulatedAction(ActionType actionType, ActionChecker actionChecker, Environment env) {
  ...
  protected abstract void innerAct(IActionParameters params, TrooperModel self);

  public void act(IActionParameters params, TrooperModel self) throws InvalidActionException {
    if (!isValid(params, self))
      throw new InvalidActionException(actionType);
    else {
      self.setActionPoints(self.getActionPoints() - cost(self));
      innerAct(params, self);
    }
  }
